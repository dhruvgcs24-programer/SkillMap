...
public class VcsHostingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_vcs_hosting);
        ...
        setup(R.id.topic1, R.id.check1, "vcs1", "GitLab", R.id.expandable1);
        setup(R.id.topic2, R.id.check2, "vcs2", "GitHub", R.id.expandable2);
        ...
        checkMarkAll.setOnCheckedChangeListener((buttonView, isChecked) -> {
            ...
            for (int i = 0; i < topicCheckBoxes.size(); i++) {
                ...
                prefs.edit().putBoolean("vcs" + (i + 1), isChecked).apply();
            }
            updateProgress();
    ...
    private void updateProgress() {
        int count = 0;
        if (prefs.getBoolean("vcs1", false)) count++;
        if (prefs.getBoolean("vcs2", false)) count++;

        progressBar.setProgress(count);
        progressCount.setText(count + "/2 Completed");
        int percent = (int) ((count / 2.0) * 100);
