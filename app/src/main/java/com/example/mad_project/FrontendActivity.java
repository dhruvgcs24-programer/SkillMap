...
public class FrontendActivity extends AppCompatActivity {
    ...
    private ImageView lockBadge2, lockBadge3, lockBadge4, lockBadge5, lockBadge6, lockBadge7, lockBadge8, lockBadge9;
    private CardView activeBadge2, activeBadge3, activeBadge4, activeBadge5, activeBadge6, activeBadge7, activeBadge8, activeBadge9;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        lvl2.setOnClickListener(v -> { if (isLevel2Unlocked) openModule(LearningModuleActivity.MODE_HTML, null); else showLockedToast(); });
        lvl3.setOnClickListener(v -> { if (isLevel3Unlocked) openModule(LearningModuleActivity.MODE_CSS, null); else showLockedToast(); });
        lvl4.setOnClickListener(v -> { if (isLevel4Unlocked) openModule(LearningModuleActivity.MODE_JAVASCRIPT, null); else showLockedToast(); });
        lvl5.setOnClickListener(v -> { if (isLevel5Unlocked) openModule(LearningModuleActivity.MODE_VERSION_CONTROL, null); else showLockedToast(); });
        lvl6.setOnClickListener(v -> { if (isLevel6Unlocked) openModule(LearningModuleActivity.MODE_VCS_HOSTING, null); else showLockedToast(); });
        lvl7.setOnClickListener(v -> { if (isLevel7Unlocked) openModule(LearningModuleActivity.MODE_PACKAGE_MANAGERS, null); else showLockedToast(); });
        lvl8.setOnClickListener(v -> { if (isLevel8Unlocked) openModule(LearningModuleActivity.MODE_CSS_FRAMEWORKS, null); else showLockedToast(); });
        lvl9.setOnClickListener(v -> { if (isLevel9Unlocked) openModule(LearningModuleActivity.MODE_LEARN_FRAMEWORK, null); else showLockedToast(); });
    ...
    private void showLockedToast() {
        Toast.makeText(this, "Complete previous levels first", Toast.LENGTH_SHORT).show();
    }
    ...
    private void updateUI() {
        ...
        overallProgressText.setText( (int) ((calc.levelsCompleted / 9.0) * 100) + "%");
        ...
        applyLevelState(calc.isInternetComplete, calc.isHtmlComplete, lockBadge2, activeBadge2, imgLvl2, txtLvl2, tickLvl2);
        applyLevelState(calc.isHtmlComplete, calc.isCssComplete, lockBadge3, activeBadge3, imgLvl3, txtLvl3, tickLvl3);
        applyLevelState(calc.isCssComplete, calc.isJsComplete, lockBadge4, activeBadge4, imgLvl4, txtLvl4, tickLvl4);
        applyLevelState(calc.isJsComplete, calc.isVcComplete, lockBadge5, activeBadge5, imgLvl5, txtLvl5, tickLvl5);
        applyLevelState(calc.isVcComplete, calc.isVcsComplete, lockBadge6, activeBadge6, imgLvl6, txtLvl6, tickLvl6);
        applyLevelState(calc.isVcsComplete, calc.isPmComplete, lockBadge7, activeBadge7, imgLvl7, txtLvl7, tickLvl7);
        applyLevelState(calc.isPmComplete, calc.isCssfComplete, lockBadge8, activeBadge8, imgLvl8, txtLvl8, tickLvl8);
        applyLevelState(calc.isCssfComplete, calc.isFwComplete, lockBadge9, activeBadge9, imgLvl9, txtLvl9, tickLvl9);
    }

    private void applyLevelState(boolean prerequisiteDone, boolean thisLevelDone,
                                 ImageView lockBadge, CardView activeBadge,
        ...
        if (prerequisiteDone) {
            ...
            if (thisLevelDone) {
                ...
                if (tick != null) {
                    ...
                    tick.setImageResource(R.drawable.ic_check_rounded);
                }
            } else {
                activeBadge.setVisibility(View.VISIBLE);
                img.setImageResource(R.drawable.circle_purple);
